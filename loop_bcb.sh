...
die() { echo "$@" ; exit 1; }
## todo don't rely on time from rpi
...
cp pump_history.json.new pump_history.json
...
grep timestamp pump_history.json.new -q && cp pump_history.json.new pump_history.json
